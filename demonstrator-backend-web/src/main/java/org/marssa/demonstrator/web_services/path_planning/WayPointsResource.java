...
public class WayPointsResource extends BaseResource {
	...
	public Representation acceptItem(JsonRepresentation entity)
			throws OutOfRange, JSONException {
		...
		for (int i = 0; i < size; i++) {
			...
			if (!getWaypoints().containsKey(waypointName)
					&& getWaypoints().putIfAbsent(
							waypointName,
							new Waypoint(waypointID, waypointName,
									new Coordinate(new Latitude(
											new DegreesDecimal(lat)),
											new Longitude(new DegreesDecimal(
													lon))))) == null) {
