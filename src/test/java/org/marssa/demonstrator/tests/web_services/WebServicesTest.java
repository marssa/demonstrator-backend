...
public class WebServicesTest {
	...
	public static void main(java.lang.String[] args) {
	    ...
	    component.getDefaultHost().attach("/lighting", new LightControllerTestApplication(cacheDirectives));
	    
	    // Attach the motor control application
	    component.getDefaultHost().attach("/motor", new MotorControllerTestApplication(cacheDirectives, motorController));

	    // Attach the rudder control application
	    component.getDefaultHost().attach("/rudder", new RudderControllerTestApplication(cacheDirectives));
	    
	    // Attach the GPS receiver application
	    component.getDefaultHost().attach("/gps", new GPSReceiverTestApplication(cacheDirectives));
	    
	    // Attach the motion control feedback application 
	    component.getDefaultHost().attach("/motionControlPage", new MotionControlPageTestApplication(cacheDirectives, motorController));
	    
	    // Attach the motion control feedback application
	    component.getDefaultHost().attach("/lightControlPage", new LightControlPageTestApplication(cacheDirectives));
	    
	    // Attach the path planner application
	    component.getDefaultHost().attach(
				"/pathPlanner",
				new PathControllerTestApplication(cacheDirectives, pathPlanningController));
