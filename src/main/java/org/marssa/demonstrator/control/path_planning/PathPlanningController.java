...
public class PathPlanningController extends MTimerTask {
	...
	public void run() {
		try {
			...
			if (arrive && endOfTrip()) // If we have arrived and its the end of
										// the trip (no more way points)
			{
				motorController.stop();
				logger.info("Kill Engines");
				timer.cancel();
				wayPointList = new ArrayList<Waypoint>();
			} else if (arrive && !endOfTrip()) // if we have arrived at our next
												// way point but its not the end
												// of the trip
			{
				logger.info("Arrived....next waypoint");
				count++;
				setNextHeading(wayPointList.get(count).getCoordinate()); // we
																			// get
																			// the
																			// next
																			// way
				...
				readPosition();
				drive();// else if we are on our way to the next way point we
						// continue driving the vessel
			}
	...
	public void startFollowingPath() throws NoConnection, InterruptedException,
		...
		setNextHeading(wayPointList.get(count).getCoordinate()); // we set the
																	// next way
																	// point to
																	// the first
																	// in the
																	// list
		timer.addSchedule(this, 0, 10); // we create the timer schedule for
										// every 1 sec.
	}
