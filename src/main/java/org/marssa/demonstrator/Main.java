...
public class Main extends ServerResource {
	...
	public static void main(java.lang.String[] args) {
		...
		try {
			...
			underwaterLightsController = new UnderwaterLightsController(
					labJacku3, LabJack.FIO13_DIR_ADDR);
			...
			// pathPlanningController = new PathPlanningController(null,
			// null,null);
			logger.info("Path Planning controller initialised successfully");

			logger.info("Initialising web services ... ");
			webServices = new WebServices(null, null, motorController,
		...
		/*
		 * // Initialise Controllers and Receivers try {
		 * 
		 * logger.info("Initialising Path Planning controller ... ");
		 * //pathPlanningController = new
		 * PathPlanningController(motorController,rudderController,gpsReceiver);
		 * pathPlanningController = new PathPlanningController(null, null,null);
		 * logger.info("Path Planning controller initialised successfully");
		 * //---------------------------------CODE FOR TESTING PATH
		 * PLANNING---------------------------------- ArrayList<Waypoint>
		 * wayPointList = new ArrayList<Waypoint>(); wayPointList.add(new
		 * Waypoint("","",new Coordinate(new Latitude(new
		 * DegreesDecimal(35.983267)) , new Longitude(new
		 * DegreesDecimal(14.387419))))); wayPointList.add(new
		 * Waypoint("","",new Coordinate(new Latitude(new
		 * DegreesDecimal(36.000184)) , new Longitude(new
		 * DegreesDecimal(14.376021))))); wayPointList.add(new
		 * Waypoint("","",new Coordinate(new Latitude(new
		 * DegreesDecimal(36.010598)) , new Longitude(new
		 * DegreesDecimal(14.391477))))); wayPointList.add(new
		 * Waypoint("","",new Coordinate(new Latitude(new
		 * DegreesDecimal(36.024835)) , new Longitude(new
		 * DegreesDecimal(14.339797))))); wayPointList.add(new
		 * Waypoint("","",new Coordinate(new Latitude(new
		 * DegreesDecimal(36.021192)) , new Longitude(new
		 * DegreesDecimal(14.318054))))); wayPointList.add(new
		 * Waypoint("","",new Coordinate(new Latitude(new
		 * DegreesDecimal(36.015387)) , new Longitude(new
		 * DegreesDecimal(14.322254)))));
		 * 
		 * pathPlanningController.setPathList(wayPointList);
		 * pathPlanningController.setTestCurrent(new Coordinate(new Latitude(new
		 * DegreesDecimal(35.971663)) , new Longitude(new
		 * DegreesDecimal(14.358312))),270);
	 ...
	 * 
	 * // MotorControl Tests try { motorController.rampTo(desiredValue); } catch
	 * (InterruptedException e) { e.printStackTrace(); }
	 * 
	 * // Rudder Tests try { rudderController.rotate(new MBoolean (true));
	 * rudderController.rotate(new MBoolean (false));
	 * rudderController.rotate(new MBoolean (true)); rudderController.rotate(new
	 * MBoolean (false)); rudderController.rotate(new MBoolean (false));
	 * rudderController.rotate(new MBoolean (true)); rudderController.rotate(new
	 * MBoolean (true)); } catch (InterruptedException e) { e.printStackTrace();
	 * }
	 * 
	 * // GPSReceiver Tests try { System.out.println("The GPS coordinates are "
	 ...
	 * gpsReceiver.getDate()); } catch (NoConnection e) { e.printStackTrace(); }
	 * catch (NoValue e) { e.printStackTrace(); }
	 */
}
