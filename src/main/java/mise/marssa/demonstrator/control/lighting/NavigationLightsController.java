...
public class NavigationLightsController implements ILightToggle{
	...
	public NavigationLightsController(MString host, MInteger port, MInteger navLights) throws UnknownHostException, NoConnection {
		...
		navigationLightLogger.info("An instance of Navigation light controller was instantiated with labjack host {}, and port {}.",poho);
		this.lightState = false;
		this.navLights = navLights;
		
	}
	
	public NavigationLightsController(MString host, MInteger port, MInteger navLights, MBoolean newState) throws NoConnection, UnknownHostException {
		// TODO There must be something wrong here. FIO4 is operating in output mode, regardless of FIO4-dir
		// Set direction for FIO4 port
		//lj.write(LabJack.FIO4_DIR_ADDR, new MBoolean(true));
		this.lj = LabJack.getInstance(host, port);
		navigationLightLogger.info("An instance of Navigation light controller was instantiated with labjack host: {}, and port: {}, with state set to: {}",poho,newState.getValue());
	...
	public void toggleLight() throws NoConnection {
		...
		navigationLightLogger.trace("NavigationLights from labjack with host: {} and port: {} have been switched {} .",poho,switched);
		lj.write(navLights, new MBoolean(lightState));
	}
	
	public MBoolean getNavigationLightState() {
		navigationLightLogger.trace(MMarker.GETTER,"Returning LightState {}, from labjack with host: {} and port: {} .",new MBoolean(lightState),poho);
	...
	public void setNavigationLightState(MBoolean newState) throws NoConnection{
		...
		if(newState.getValue())
			...
			switched = "OFF";
		navigationLightLogger.trace(MMarker.SETTER,"Switching Navigationlight : {} ,from labjack with host: {} and port: {}",switched,poho);
		lj.write(navLights, newState);
	}
}
